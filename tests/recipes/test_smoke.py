...
def test_pretrain_causal_config_loads():
    ...
    cfg = load_config(REPO / "examples" / "references" / "recipes" / "pretrain_causal.yaml")
...
def test_sft_chat_dry_run():
    bundle = build_prep_runner(REPO / "examples" / "references" / "recipes" / "sft_chat.yaml")
...
def test_vlm_sft_dry_run():
    bundle = build_prep_runner(REPO / "examples" / "references" / "recipes" / "vlm_sft.yaml")
    plan = bundle["runner"].dry_run()
    names = [p.name for p in plan]
    assert "train_data" in names
...
def test_sft_chat_run_then_replay(tmp_path: Path):
    bundle = build_prep_runner(
        REPO / "examples" / "references" / "recipes" / "sft_chat.yaml",
...
def test_sft_chat_hf_dry_run():
    ...
    bundle = build_prep_runner(REPO / "examples" / "references" / "recipes" / "sft_chat_hf.yaml")
    plan = bundle["runner"].dry_run()

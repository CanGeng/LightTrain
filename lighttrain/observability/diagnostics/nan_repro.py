...
The repro script depends only on torch + safetensors + lighttrain.observability.minimal
...
Only depends on torch + safetensors + lighttrain.observability.minimal (~80 LoC each).
"""
...
def _infer_spec(model: torch.nn.Module) -> dict[str, Any]:
    ...
    from lighttrain.registry import contains
